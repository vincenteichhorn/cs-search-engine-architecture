...
def test_disk_array_in_memory(tmp_path_factory):
    ...
    id = disk_array.append(bytearray(data1), 0, len(data1))
    assert id == 0
    id = disk_array.append(bytearray(data2), 0, len(data2))
    assert id == 1

    retrieved1 = disk_array.get(0).to_bytearray()
    retrieved2 = disk_array.get(1).to_bytearray()
...
def test_disk_array_on_disk_and_memory(tmp_path_factory):
    ...
    id = disk_array.append(bytearray(data2), 0, len(data2))
    assert id == 1
    id = disk_array.append(bytearray(data3), 0, len(data3))
    assert id == 2

    retrieved1 = disk_array.get(0).to_bytearray()
    retrieved2 = disk_array.get(1).to_bytearray()
    retrieved3 = disk_array.get(2).to_bytearray()
...
def test_disk_array_out_of_bounds(tmp_path_factory):
    ...
    id = disk_array.append(bytearray(data), 0, len(data))
    assert id == 0

    with pytest.raises(IndexError):
        disk_array.get(1)  # Accessing out of bounds
